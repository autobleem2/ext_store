...
//               image URL only for a game none of them knows
...
public:
    ...
    // the lookup itself, on the calling thread (the worker's, and the tests')
...
private:
    ...
    std::string gameCover(const Request &request);
    bool online() const;
